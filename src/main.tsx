...
function PlayScreen({ gameId, gameInsights, traceComplete, promptRunProgress, learningTourComplete, exerciseProgress, setGameId, onGlossary, onInsight, onExerciseAttempt, onPromptRunAttempt, onTraceComplete, onLearningTourComplete }) {
  ...
  const startChallenge = useCallback((item) => {
    ...
    document.querySelector<HTMLElement>('.pl-shell')?.scrollTo({ top: 0, behavior: 'auto' })
  ...
  if (gameId === 'trace-one-prompt' || gameId === 'prompt-run') {
    return (
      <PromptRunScreen
        ...
        onAttempt={() => {
          recordChallengeStart('prompt-run', {
            outcome: 'Prompt Run capstone attempt started. Progress saved on this device.',
            misconceptionTags: ['one-token-at-a-time']
          })
        }}
        onProgress={(outcome) => updateChallenge('prompt-run', outcome)}
        onComplete={(outcome = {}) => {
          completeChallenge('prompt-run', {
            progressPct: 100,
            outcome: 'Completed. You traced the full inference loop.',
            misconceptionTags: [
              'token-id-is-not-meaning',
              'embedding-is-representation',
              'probability-is-not-truth',
              'one-token-at-a-time',
              'context-grows-and-expires',
              'model-does-not-remember-forever'
            ],
            ...outcome
          })
          onTraceComplete()
...
const promptRunStations = [
  {
    id: 'separate-prompt-response',
    legacyStepId: 'prompt-run-prompt-response',
    title: 'Separate prompt from response',
    shortTitle: 'Prompt',
    objective: 'Identify what was given to the model and what the model generated.',
    instruction: 'Choose the card that correctly separates the user prompt from the generated response.',
    mode: 'choice',
    glossary: ['prompt', 'response'],
    concepts: ['prompt', 'response', 'context'],
    tags: ['one-token-at-a-time'],
    visual: {
      prompt: canonicalPromptResponse.userPrompt,
      response: `${canonicalPromptResponse.responseSoFar} ${canonicalPromptResponse.chosenNextToken}`
    },
    choices: [
      {
        id: 'given-vs-generated',
        label: 'Prompt is given; response tokens are generated.',
        correct: true,
        feedback: 'Good trace. This is the current context: the prompt was supplied, and response tokens are generated one at a time.'
      },
      {
        id: 'both-generated',
        label: 'The prompt and response were both generated by the model.',
        feedback: 'This choice reveals a common mix-up. The user prompt is input context; only the response tokens are generated.'
      },
      {
        id: 'prompt-is-memory',
        label: 'The prompt became permanent memory before the answer.',
        feedback: 'This choice reveals a common mix-up. Context is temporary input, not a durable memory update.'
      }
    ]
  },
  {
    id: 'tokenize',
    legacyStepId: 'prompt-run-tokenizer',
    title: 'Tokenize',
    shortTitle: 'Tokens',
    objective: 'Build a simplified token view of the prompt text.',
    instruction: 'Tap the token-like pieces in order. This is simplified; real tokenizer boundaries can differ.',
    mode: 'sequence',
    glossary: ['tokenization', 'token'],
    concepts: ['token pieces', 'simplified view'],
    tags: ['one-token-at-a-time'],
    sourceText: 'Describe two pets in conflict.',
    expectedOrder: ['describe', 'two-pets', 'conflict'],
    tokens: [
      { id: 'two-pets', label: 'two pets' },
      { id: 'conflict', label: 'conflict' },
      { id: 'describe', label: 'Describe' }
    ],
    success: 'Good trace. The text has become token pieces.',
    review: 'This choice reveals a common mix-up. Keep the simplified pieces in the same reading order.'
  },
  {
    id: 'map-to-numbers',
    legacyStepId: 'prompt-run-token-ids',
    title: 'Map to numbers',
    shortTitle: 'IDs',
    objective: 'See that token IDs are lookup numbers, not meanings by themselves.',
    instruction: 'Match the highlighted token to its lookup ID.',
    mode: 'id-match',
    glossary: ['token-id', 'embedding'],
    concepts: ['token', 'ID', 'lookup'],
    tags: ['token-id-is-not-meaning', 'embedding-is-representation'],
    targetToken: 'dog',
    pairs: [
      { token: 'dog', id: '421' },
      { token: 'cat', id: '982' },
    ...
    choices: [
      { id: '421', label: 'ID 421', correct: true, feedback: 'Good trace. Token IDs are lookup numbers, not meanings by themselves.' },
      { id: '982', label: 'ID 982', feedback: 'This choice reveals a common mix-up. That lookup number belongs to a different token.' },
      { id: 'meaning', label: 'meaning: animal', feedback: 'This choice reveals a common mix-up. An ID is a lookup number, not a definition.' }
    ]
  },
  {
    id: 'carry-through-layers',
    legacyStepId: 'prompt-run-hidden-state',
    title: 'Carry context through layers',
    shortTitle: 'Hidden',
    objective: 'Track how transformer layers update temporary context-shaped representations.',
    instruction: 'Choose what the context bundle becomes as it moves through layers.',
    mode: 'layers',
    glossary: ['hidden state', 'attention'],
    concepts: ['embeddings', 'layers', 'hidden states'],
    tags: ['embedding-is-representation', 'model-does-not-remember-forever'],
    rails: [
      { label: 'Embeddings', note: 'learned starting vectors' },
      { label: 'Layer 1', note: 'context begins to mix' },
      { label: 'Layer 2', note: 'hidden states carry updated context' }
    ],
    choices: [
      {
        id: 'temporary-hidden-states',
        label: 'Temporary hidden states shaped by context.',
        correct: true,
        feedback: 'Good trace. Hidden states carry context-shaped information forward.'
      },
      {
        id: 'durable-weight-change',
        label: 'Durable weight updates inside the model.',
        feedback: 'This choice reveals a common mix-up. Inference uses weights; it does not durably update them.'
      },
      {
        id: 'retrieved-truth',
  ...
  {
    id: 'decision-room',
    legacyStepId: 'prompt-run-sampling',
    title: 'Decision room',
    shortTitle: 'Probs',
    objective: 'Choose one next token from probability-shaped candidates.',
    instruction: 'Inspect the candidate bars and choose the likely next token. Probability is not truth.',
    mode: 'probability',
    glossary: ['logits', 'softmax', 'sampling'],
    concepts: ['logits', 'probabilities', 'sampling'],
    tags: ['probability-is-not-truth', 'one-token-at-a-time'],
    fragment: `${canonicalPromptResponse.responseSoFar} ...`,
    tokens: [
      { id: 'floor', label: 'floor', probability: 67, correct: true },
      { id: 'room', label: 'room', probability: 18 },
      { id: 'quantum', label: 'quantum', probability: 2 },
      { id: 'truth', label: 'truth', probability: 1 }
    ],
    choices: [
      { id: 'floor', label: 'Choose floor', correct: true, feedback: 'Good trace. Probabilities shape the next choice, and one token is sampled or selected.' },
      { id: 'truth', label: 'Choose truth because it sounds certain', feedback: 'This choice reveals a common mix-up. Probabilities are fit-to-context signals, not proof.' },
      { id: 'all-at-once', label: 'Generate the whole answer at once', feedback: 'This choice reveals a common mix-up. Generation is one token, append, repeat.' }
    ]
  },
  {
    id: 'append-repeat',
    legacyStepId: 'prompt-run-append-repeat',
    title: 'Append and repeat',
    shortTitle: 'Repeat',
    objective: 'Explain what changed after one next token was chosen.',
    instruction: 'Choose the explanation that prepares the next model run.',
    mode: 'append',
    glossary: ['autoregression', 'context window'],
    concepts: ['append', 'context grows', 'repeat'],
    tags: ['context-grows-and-expires', 'model-does-not-remember-forever'],
    beforeTokens: ['prompt', 'A', 'jealous', 'dog'],
    chosenToken: 'floor',
    afterTokens: ['prompt', 'A', 'jealous', 'dog', 'floor'],
    choices: [
      {
        id: 'context-grew',
        label: 'Context grew by one token; the next run sees the expanded context.',
        correct: true,
        feedback: 'Good trace. One token is appended, then the day repeats.'
      },
      {
        id: 'weights-changed',
        label: 'The model permanently learned the new token.',
        feedback: 'This choice reveals a common mix-up. Inference does not durably change weights.'
      },
      {
        id: 'finished-all-response',
        label: 'The whole response appeared at once.',
        feedback: 'This choice reveals a common mix-up. Autoregressive generation appends one token and runs again.'
      }
    ]
  }
]

function getPromptRunStationTags(station, result) {
  if (!result) return station.tags ?? []
  return uniqueList([
    ...(station.tags ?? []),
    !result.ok ? station.id === 'map-to-numbers' ? 'token-id-is-not-meaning' : '' : '',
    !result.ok ? station.id === 'decision-room' ? 'probability-is-not-truth' : '' : '',
    !result.ok ? station.id === 'append-repeat' ? 'model-does-not-remember-forever' : '' : ''
  ])
}

function getPromptRunStationResult(station, selectedId, sequence) {
  if (station.mode === 'sequence') {
    if (sequence.length < station.expectedOrder.length) return null
    const ok = station.expectedOrder.every((tokenId, index) => sequence[index] === tokenId)
    return {
      stationId: station.id,
      selectedId: sequence.join(' > '),
      ok,
      feedback: ok ? station.success : station.review
    }
  }
  const choice = station.choices?.find((item) => item.id === selectedId)
  if (!choice) return null
  return {
    stationId: station.id,
    selectedId: choice.id,
    ok: Boolean(choice.correct),
    feedback: choice.feedback
  }
}

function PromptRunScreen({ onBack, onAttempt, onProgress, onComplete, saved, promptRunProgress, onPromptRunAttempt, onGlossary }) {
  ...
  const [feedback, setFeedback] = useState({
    tone: 'neutral',
    message: 'Start with the current context. A prompt is given; response tokens are generated.'
  })
  const station = promptRunStations[stationIndex]
  const stationNumber = stationIndex + 1
  const resultForStation = stationResults.find((result) => result.stationId === station.id)
  const pendingResult = getPromptRunStationResult(station, selectedId, sequence)
  const activeResult = resultForStation ?? pendingResult
  const actionReady = Boolean(activeResult)
  const reviewCount = stationResults.filter((result) => !result.ok).length
  const criticalReview = stationResults.some((result) => !result.ok && ['map-to-numbers', 'decision-room', 'append-repeat'].includes(result.stationId))
  const reviewSuggested = reviewCount >= 2 || criticalReview
  const completedLegacy = Boolean(saved || promptRunProgress?.finalChallengeComplete)
  const currentProgress = completed
    ? 100
    : Math.min(99, Math.round(((stationIndex + (actionReady ? 0.75 : 0)) / promptRunStations.length) * 100))
  const status = completed
    ? reviewSuggested ? 'review-suggested' : 'completed'
    : 'in-progress'
  const statusLabel = completed
    ? reviewSuggested ? 'Review suggested' : 'Completed'
    : `Station ${stationNumber} of ${promptRunStations.length}`
  ...
  useEffect(() => {
    ...
    setFeedback({
      tone: 'neutral',
      message: stationIndex === 0
        ? 'Start with the current context. A prompt is given; response tokens are generated.'
        : 'Trace the next station in the prompt loop.'
    })
    requestAnimationFrame(() => {
      document.querySelector<HTMLElement>('.pl-shell')?.scrollTo({ top: 0, behavior: prefersReducedMotion() ? 'auto' : 'smooth' })
      document.getElementById('prompt-run-station-title')?.focus?.({ preventScroll: true })
    })
  }, [stationIndex])

  function saveStationResult(result) {
    const nextResults = [
      ...stationResults.filter((item) => item.stationId !== station.id),
      result
    ]
    setStationResults(nextResults)
    onPromptRunAttempt(station.legacyStepId, {
      correct: result.ok,
      revealed: false,
    ...
    onProgress({
      status: 'in-progress',
      progressPct: Math.min(99, Math.round(((stationIndex + 0.75) / promptRunStations.length) * 100)),
      outcome: result.ok ? 'Good trace. Prompt Run progress saved.' : 'Review suggested. Prompt Run progress saved.',
      misconceptionTags: getPromptRunStationTags(station, result)
    })
    setFeedback({
      tone: result.ok ? 'good' : 'review',
      message: result.feedback
    })
    return nextResults
  }

  function chooseStationChoice(choiceId) {
    if (completed) return
    setSelectedId(choiceId)
    const result = getPromptRunStationResult(station, choiceId, sequence)
    if (result) saveStationResult(result)
  }

  function chooseSequenceToken(tokenId) {
    if (completed || sequence.includes(tokenId) || resultForStation) return
    const nextSequence = [...sequence, tokenId]
    setSequence(nextSequence)
    const result = getPromptRunStationResult(station, selectedId, nextSequence)
    if (result) saveStationResult(result)
  }

  function resetStation() {
    setStationResults((prev) => prev.filter((result) => result.stationId !== station.id))
    setSelectedId(null)
    setSequence([])
    setFeedback({
      tone: 'neutral',
      message: station.mode === 'sequence'
        ? 'Tap the simplified token pieces in reading order.'
        : 'Try this station again. Progress is saved on this device.'
    })
  }

  function getCurrentResults() {
    if (stationResults.some((result) => result.stationId === station.id)) return stationResults
    if (!pendingResult) return stationResults
    return [...stationResults, pendingResult]
  }

  function finishChallenge(nextResults = stationResults) {
    setStationResults(nextResults)
    const finalReviewCount = nextResults.filter((result) => !result.ok).length
    const finalCriticalReview = nextResults.some((result) => !result.ok && ['map-to-numbers', 'decision-room', 'append-repeat'].includes(result.stationId))
    const finalReviewSuggested = finalReviewCount >= 2 || finalCriticalReview
    const tags = uniqueList(nextResults.flatMap((result) => {
      const completedStation = promptRunStations.find((item) => item.id === result.stationId) ?? station
      return getPromptRunStationTags(completedStation, result)
    }))
    setCompleted(true)
    onPromptRunAttempt(PROMPT_RUN_FINAL_ID, {
      correct: true,
      revealed: false,
      answer: finalReviewSuggested ? 'prompt-run-v2-review-suggested' : 'prompt-run-v2-full-loop-traced'
    })
    onComplete({
      progressPct: 100,
      reviewSuggested: finalReviewSuggested,
      outcome: finalReviewSuggested
        ? 'Completed. Review suggested. Revisit the loop pieces that felt slippery.'
        : 'Completed. You traced the full inference loop.',
      misconceptionTags: tags.length ? tags : promptRunV2Tags
    })
  }
  ...
  function goNext() {
    ...
    if (!actionReady) {
      setFeedback({ tone: 'review', message: 'This choice reveals a common mix-up. Try one action before moving on.' })
      return
    }
    if (stationIndex >= promptRunStations.length - 1) {
      finishChallenge(currentResults)
      return
    }
    setStationIndex((prev) => Math.min(promptRunStations.length - 1, prev + 1))
  }
  ...
  function restartChallenge() {
    ...
    setSequence([])
    setCompleted(false)
    setFeedback({ tone: 'neutral', message: 'New Prompt Run attempt started. Progress saved on this device.' })
  }

  return (
    <PlayChallengeShell
      title="Prompt Run"
      titleId="prompt-run-title"
      eyebrow="Capstone challenge"
      subtitle="Trace one prompt through the full inference loop."
      onBack={onBack}
      className="prompt-run-screen prompt-run-v2-screen"
      data-prompt-run-station={station.id}
      headerChildren={
        <>
          <div className="prompt-run-v2-status-row">
            <PlayStatusPill status={status} label={statusLabel} />
            <span>{reviewCount} review signal{reviewCount === 1 ? '' : 's'}</span>
          </div>
          <PlayProgressRail value={currentProgress} label={`${currentProgress}% Prompt Run progress`} />
        </>
      }
    >
      {completed ? (
        <PlayCompletionPanel
          title={reviewSuggested ? 'Review suggested' : 'Insight unlocked'}
          titleId="prompt-run-completion-title"
          className="prompt-run-v2-completion"
        >
          <p><strong>Completed.</strong> You traced prompt to tokens, IDs, embeddings, hidden states, probabilities, one sampled token, append, and repeat.</p>
          {reviewSuggested ? (
          ...
          <PlayActionRow>
            ...
            <button className="secondary-btn" type="button" onClick={onBack}>Back to Play</button>
            <button className="text-btn" type="button" onClick={() => onGlossary('autoregression')}>Autoregression</button>
          </PlayActionRow>
        </PlayCompletionPanel>
      ) : (
        <>
          ...
          <PlayChallengeBoard label="Prompt Run capstone board" className="prompt-run-v2-board">
            <ol className="prompt-run-day-rail" aria-label="Prompt Run stations">
              {promptRunStations.map((item, index) => (
                <li
                  key={item.id}
                  className={[
                    index === stationIndex ? 'is-active' : '',
                    stationResults.some((result) => result.stationId === item.id) ? 'is-done' : ''
                  ].filter(Boolean).join(' ')}
                >
                  <span>{index + 1}</span>
            ...
            <section className="prompt-run-station-card" aria-labelledby="prompt-run-station-title">
              <span className="step-label">Station {stationNumber}</span>
              <h2 id="prompt-run-station-title" tabIndex={-1}>{station.title}</h2>
              <p>{station.objective}</p>
              <p className="prompt-run-station-instruction">{station.instruction}</p>
              <div className="play-token-row" aria-label={`${station.title} related concepts`}>
                {station.concepts.map((concept) => (
                  <PlayTokenChip key={concept} tone={concept.includes('prob') || concept === 'logits' ? 'probability' : concept.includes('context') ? 'context' : 'prompt'}>
                    {concept}
                  </PlayTokenChip>
                ))}
              </div>
            ...
            <PromptRunStationInteraction
              station={station}
              selectedId={selectedId}
              sequence={sequence}
              onChoose={chooseStationChoice}
              onChooseSequence={chooseSequenceToken}
            />
          </PlayChallengeBoard>

          <PlayFeedbackPanel tone={feedback.tone === 'good' ? 'good' : feedback.tone === 'review' ? 'review' : 'neutral'} className="prompt-run-v2-feedback">
            <p>{feedback.message}</p>
          </PlayFeedbackPanel>

          <PlayActionRow className="prompt-run-v2-actions">
            <button className="primary-btn" type="button" onClick={goNext} disabled={!actionReady}>
              {stationIndex >= promptRunStations.length - 1 ? 'Complete capstone' : 'Next station'}
            </button>
            <button className="secondary-btn" type="button" onClick={resetStation}>Reset station</button>
            <button className="text-btn" type="button" onClick={() => onGlossary(station.glossary[0])}>{station.glossary[0]}</button>
          </PlayActionRow>
          <PlayScrollHint>More loop below</PlayScrollHint>
        </>
      )}
    </PlayChallengeShell>
  )
...
function PromptRunStationInteraction({ station, selectedId, sequence, onChoose, onChooseSequence }) {
  if (station.mode === 'sequence') {
    return (
      <section className="prompt-run-token-station" aria-labelledby="prompt-run-token-title">
        <h3 id="prompt-run-token-title">Simplified token view</h3>
        <p className="prompt-run-source-text">{station.sourceText}</p>
        <div className="prompt-run-sequence-tray" aria-label="Selected token order">
          {sequence.length ? sequence.map((id, index) => {
            const token = station.tokens.find((item) => item.id === id)
            return <span key={`${id}-${index}`}>{token?.label ?? id}</span>
          }) : <span className="is-placeholder">Tap chips below</span>}
        </div>
        <div className="prompt-run-token-bank">
          {station.tokens.map((token) => (
            <button
              key={token.id}
              type="button"
              className={sequence.includes(token.id) ? 'is-selected' : ''}
              onClick={() => onChooseSequence(token.id)}
              disabled={sequence.includes(token.id)}
            >
              {token.label}
            </button>
          ))}
  ...
  if (station.mode === 'id-match') {
    return (
      <section className="prompt-run-id-station" aria-labelledby="prompt-run-id-title">
        <h3 id="prompt-run-id-title">Lookup table</h3>
        <div className="prompt-run-id-table" aria-label="Token ID examples">
          {station.pairs.map((pair) => (
            <span key={pair.token} className={pair.token === station.targetToken ? 'is-target' : ''}>
              <strong>{pair.token}</strong>
              <small>ID {pair.id}</small>
            </span>
          ))}
        </div>
        <PromptRunChoiceList station={station} selectedId={selectedId} onChoose={onChoose} />
      </section>
    )
  }

  if (station.mode === 'layers') {
    return (
      <section className="prompt-run-layer-station" aria-labelledby="prompt-run-layer-title">
        <h3 id="prompt-run-layer-title">Layer rails</h3>
        <div className="prompt-run-layer-rails">
          {station.rails.map((rail, index) => (
            <span key={rail.label}>
              <strong>{index + 1}. {rail.label}</strong>
  ...
  if (station.mode === 'probability') {
    return (
      <section className="prompt-run-probability-station" aria-labelledby="prompt-run-probability-title">
        <h3 id="prompt-run-probability-title">Candidate probabilities</h3>
        <p className="prompt-run-fragment">{station.fragment}</p>
        <div className="prompt-run-probability-list">
          {station.tokens.map((token) => (
            <span key={token.id} className={token.correct ? 'is-likely' : ''}>
              <strong>{token.label}</strong>
              <span className="prompt-run-probability-track" aria-label={`${token.label} probability ${token.probability}%`}>
                <span style={{ width: `${token.probability}%` }} />
              </span>
              <em>{token.probability}%</em>
            </span>
          ))}
        </div>
        <PromptRunChoiceList station={station} selectedId={selectedId} onChoose={onChoose} />
  ...
  if (station.mode === 'append') {
    return (
      <section className="prompt-run-append-station" aria-labelledby="prompt-run-append-title">
        <h3 id="prompt-run-append-title">Append tray</h3>
        <div className="prompt-run-append-trays">
          <span>
            <strong>Before</strong>
            <small>{station.beforeTokens.join(' · ')}</small>
          </span>
          <span className="is-chosen">
            <strong>Chosen token</strong>
            <small>{station.chosenToken}</small>
          </span>
          <span>
            <strong>Next context</strong>
            <small>{station.afterTokens.join(' · ')}</small>
          </span>
        </div>
        <PromptRunChoiceList station={station} selectedId={selectedId} onChoose={onChoose} />
      </section>
  ...
  return (
    <section className="prompt-run-choice-station" aria-labelledby="prompt-run-choice-title">
      <h3 id="prompt-run-choice-title">Current context</h3>
      {station.visual && (
        <div className="prompt-run-context-split">
          <span><strong>User prompt</strong><small>{station.visual.prompt}</small></span>
          <span><strong>Generated response</strong><small>{station.visual.response}</small></span>
        </div>
      )}
      <PromptRunChoiceList station={station} selectedId={selectedId} onChoose={onChoose} />
    </section>
...
function PromptRunChoiceList({ station, selectedId, onChoose }) {
  return (
    <div className="prompt-run-choice-list">
      {station.choices.map((choice) => (
        <PlayChoiceButton
          key={choice.id}
          className="prompt-run-v2-choice"
          selected={selectedId === choice.id}
