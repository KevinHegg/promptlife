...
const versionSlug = 'v0-28-9-image2-source-processing-final'
const port = Number(process.env.PROMPTLIFE_CAPTURE_PORT ?? 5198)
...
function getPlaywright() {
  ...
  throw new Error('Playwright is required to capture v0.28.7b image source-processing screenshots.')
}
...
async function setAppState(page, { tab, lessonId = null }) {
  await page.goto(`${baseUrl}/?v=0289-capture-state`, { waitUntil: 'networkidle' })
...
async function main() {
  ...
  try {
    ...
    for (const lessonId of newlyLiveAssetLessons) {
      await captureLessonVisual(page, { width: 390, lessonId, name: `${lessonId}-image2` })
      await captureLessonVisual(page, { width: 320, lessonId, name: `${lessonId}-image2` })
      await captureLessonVisual(page, { width: 1024, height: 900, lessonId, name: `${lessonId}-image2-desktop` })
    }

    for (const lessonId of allLiveAssetLessons) {
      await captureLessonFullPage(page, { width: 390, lessonId, name: `journey-card-${lessonId}` })
    }

    await captureLessonVisual(page, { width: 390, lessonId: 'collective-intelligence', name: 'already-live-collective-intelligence-smoke' })
    ...
    console.log(`Captured v0.28.9 Image 2 source-processing screenshots in ${path.relative(root, screenshotDir)}.`)
  } catch (error) {
