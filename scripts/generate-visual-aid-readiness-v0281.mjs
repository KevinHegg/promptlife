...
import { VERSION, summarizeReadiness, visualAidReadinessBriefs, visualAidTemplates } from './visual-aid-readiness-data-v0281.mjs'

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..')
const outDir = path.join(root, 'docs', 'journey', 'visual-aids')
const testingDir = path.join(root, 'docs', 'testing')
const screenshotDir = path.join(root, 'docs', 'journey', 'screenshots', 'v0-28-1')

const rubricJsonPath = path.join(outDir, 'visual-aid-readiness-rubric-v0-28-1.json')
const rubricCsvPath = path.join(outDir, 'visual-aid-readiness-rubric-v0-28-1.csv')
const rubricMdPath = path.join(outDir, 'visual-aid-readiness-rubric-v0-28-1.md')
const reviewHtmlPath = path.join(outDir, 'prompt-life-v0-28-1-all-visual-aids-review.html')
const reviewPdfPath = path.join(outDir, 'prompt-life-v0-28-1-all-visual-aids-review.pdf')
const readinessHtmlPath = path.join(outDir, 'prompt-life-v0-28-1-visual-aid-readiness-report.html')
const readinessPdfPath = path.join(outDir, 'prompt-life-v0-28-1-visual-aid-readiness-report.pdf')
const feedbackMdPath = path.join(testingDir, 'prompt-life-v0-28-1-visual-aid-human-feedback.md')
const feedbackHtmlPath = path.join(testingDir, 'prompt-life-v0-28-1-visual-aid-human-feedback.html')
const feedbackPdfPath = path.join(testingDir, 'prompt-life-v0-28-1-visual-aid-human-feedback.pdf')
const overflowAuditPath = path.join(outDir, 'visual-overflow-audit-v0-28-1.json')
...
function overflowIssueCount(audit) {
  if (!audit) return 0
  if (Array.isArray(audit.issues)) return audit.issues.length
  const summary = audit.summary ?? {}
  return Object.values(summary).reduce((count, value) => count + (typeof value === 'number' ? value : value ? 1 : 0), 0)
}

function imagePath(brief) {
  return `../screenshots/v0-28-1/${brief.currentVisualId}-390.png`
}

function imagePath320(brief) {
  return `../screenshots/v0-28-1/${brief.currentVisualId}-320.png`
}

function hasScreenshot320(brief) {
  return existsSync(path.join(screenshotDir, `${brief.currentVisualId}-320.png`))
}

async function readOverflowAudit() {
  if (!existsSync(overflowAuditPath)) return null
  return JSON.parse(await readFile(overflowAuditPath, 'utf8'))
}
...
function exportPdf(htmlPath, pdfPath) {
  ...
  if (!chrome) {
    throw new Error('Could not find Chrome/Chromium to export PDF.')
  }
  ...
  if (result.status !== 0) {
    throw new Error(`Failed to export PDF for ${htmlPath}`)
  }
}
...
function renderFullReviewHtml(payload) {
    ...
    <article class="brief-card" id="${brief.currentVisualId}">
      ...
      <div class="shot-pair">
        <figure>
          ...
          <figcaption>390px: ${brief.currentVisualTitle}</figcaption>
        </figure>
        ${hasScreenshot320(brief) ? `<figure>
          <img src="${imagePath320(brief)}" alt="320px screenshot of ${brief.currentVisualTitle}" />
          <figcaption>320px overflow spot-check</figcaption>
        </figure>` : ''}
      </div>
      <p class="issue">${brief.currentIssueSummary}</p>
      <dl>
  ...
  return renderHtmlShell({
    title: 'Prompt Life v0.28.1 All Visual Aids Review',
    body: `
      <p class="lede">This packet lets reviewers inspect all 39 Journey visual aids without clicking through the app. It includes each card's 390px screenshot, 320px overflow check where captured, template, concept, misconception, caption/callouts, score, priority, and tester note.</p>
...
function renderReadinessReportHtml(payload) {
  ...
  return renderHtmlShell({
    title: 'Prompt Life v0.28.1 Visual Aid Readiness Report',
    body: `
      ...
      ${renderMetricGrid(payload)}
      ${payload.overflowAudit ? `<h2>Overflow Audit</h2><p><strong>${payload.overflowAudit.status}</strong>: checked ${payload.overflowAudit.visualAidCount ?? payload.overflowAudit.totalVisuals} visual aids at ${payload.overflowAudit.widths.join('px and ')}px. Issues found: ${overflowIssueCount(payload.overflowAudit)}.</p>` : ''}
      ...
      <ul>
        ...
        <li>Created a full all-visuals review packet and tester feedback sheet.</li>
...
<head>
  ...
  <style>
    ...
    .issue { background: #effafa; border-left: 4px solid #00a7a7; padding: 10px 12px; border-radius: 10px; font-weight: 800; color: #174653; }
...
function renderFeedbackMarkdown() {
  return `# Prompt Life v0.28.1 Visual Aid Human Feedback Sheet
...
function renderFeedbackHtml(markdown) {
  const sections = markdown
    ...
    .join('\n')
  return renderHtmlShell({ title: 'Prompt Life v0.28.1 Visual Aid Human Feedback Sheet', body: sections.replace('<h1>Prompt Life v0.28.1 Visual Aid Human Feedback Sheet</h1>', '') })
}
...
async function main() {
  ...
  const overflowAudit = await readOverflowAudit()
  const payload = {
    version: VERSION,
  ...
  exportPdf(reviewHtmlPath, reviewPdfPath)
  exportPdf(readinessHtmlPath, readinessPdfPath)
  exportPdf(feedbackHtmlPath, feedbackPdfPath)
  ...
  console.log(`Exported v${VERSION} visual-aid PDFs.`)
}
