...
import { visualAidReadinessBriefs } from './visual-aid-readiness-data-v0281.mjs'
...
async function captureVisuals(page, width) {
  ...
  await page.goto(`${baseUrl}/review/visual-aids?debug=1&v=0281-capture-${width}`, { waitUntil: 'networkidle' })
...
async function setStoredTab(page, tab) {
  await page.goto(`${baseUrl}/?debug=1&v=0281-smoke-${tab}`, { waitUntil: 'networkidle' })
...
async function captureSmokeScreens(page) {
  ...
  await page.goto(`${baseUrl}/review/visual-aids?debug=1&v=0281-desktop`, { waitUntil: 'networkidle' })
  await page.evaluate(() => document.fonts?.ready)
  await page.screenshot({
