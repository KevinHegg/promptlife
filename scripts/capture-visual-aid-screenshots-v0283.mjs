...
async function captureVisuals(page, width) {
  ...
  await page.goto(`${baseUrl}/review/visual-aids?debug=1&v=0283-capture-${width}`, { waitUntil: 'networkidle' })
...
async function setStoredTab(page, tab) {
  await page.goto(`${baseUrl}/?debug=1&v=0283-smoke-${tab}`, { waitUntil: 'networkidle' })
...
async function captureSmokeScreens(page) {
  ...
  await page.goto(`${baseUrl}/review/visual-aids?debug=1&v=0283-desktop`, { waitUntil: 'networkidle' })
  await page.evaluate(() => document.fonts?.ready)
  await page.screenshot({
